...
tic
CaseOfStudy = 2; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Caso de Estudio de Disipadores %%%%%%%%%%%%%%%%%% Ver Figura %%%%%%%%%%%%%%%%%%
%0: Sin nada
%1,2,3,4,5
...
for i_piso=1:NStory+1
    for j_vano=1:2*(NBay+1)
        ...
        if Node<=199
            ...
            Relation_Nodes(ContNode,:)=[Node (i_piso-2)*20+j_vano+1];
        end
    end
end
%% Disipadores
...
for i=1:(NBay)*NStory
    ...
    else BeamHingeLeft(j,i)%>0.0065 
        PerfBeamHingeLeft{j,i}='CP';
    end
end
        %% Histéresis - Incursión Inelástica
        pt=figure();
            hold on
    ...
    if CaseOfStudy~=0 %Casos según la configuración de los disipadores
        t=NodesVelo(:,1);
        ...
        %% Cuantificación de Energia

        t=NodesDisp(:,1);
        n=size(t,1);

        %% Energía Histerética Ea
        M=M_Spr(:,2:end);
        R=R_Spr(:,2:end);
        N=size(M,2);
        Ea=zeros(n,N);
        ...
        %% Energía Ingresada al Sistema Eeq y Energía Disipada Ed (Rayleigh)
        Eeq=zeros(n,1);
        Ed=zeros(n,1);
        Evda=zeros(n,1);
...
for i=1:(NBay+1)*NStory
    ...
    else %AveColHingeBelow(i)>0.0065;
