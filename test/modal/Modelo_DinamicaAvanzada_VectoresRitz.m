fprintf('>\tMODELO_DINAMICA_AVANZADA_VECTORES_RITZ\n');

%% Creamos el modelo
modeloObj = Modelo(2, 3);
modeloObj.definirNombre('Modelo Dinamica Avanzada Vectores Ritz');
...
restricciones{1} = RestriccionNodo('R1', nodos{1}, [2, 3]');
restricciones{2} = RestriccionNodo('R2', nodos{2}, [2, 3]');
restricciones{3} = RestriccionNodo('R3', nodos{3}, [2, 3]');
restricciones{4} = RestriccionNodo('R4', nodos{4}, [2, 3]');
restricciones{5} = RestriccionNodo('R5', nodos{5}, [2, 3]');
restricciones{6} = RestriccionNodo('R6', nodos{6}, [2, 3]');
restricciones{7} = RestriccionNodo('R7', nodos{7}, [2, 3]');
restricciones{8} = RestriccionNodo('R8', nodos{8}, [2, 3]');
...
analisisObj.analizar('nModos', 174, 'rayleighBeta', [0.02, 0.02], 'rayleighModo', [1, 8], ...
    'rayleighDir', ['h', 'h'], 'cpenzienBeta', [0.02, 0.02, 0], 'condensar', true, ...
    'valvecAlgoritmo', 'ritz', 'valvecTolerancia', 0.0001, ...
    'muIterDespl', 1.996^2 , 'nRitz', 220);
analisisObj.disp();
