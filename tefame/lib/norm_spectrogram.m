function [matrix, varargout] = norm_spectrogram(t, acc, MAX_FREQ, NORMALIZE, WINDOW_TIME, WINDOW_TIME_MOVEMENT)
...
while true
    ...
    %% Baseline correction
    acc_win = [acc_win', zeros(1, length(acc_win)*0)]';
    acc_win = detrend(acc_win, 0);
    
    %% Apply tuckey window
    tuckey = tukeywin(length(acc_win), 0.05);
